    ...
    async def create(cls, *artists):
        ...
        async with aiohttp.ClientSession(timeout=timeout) as session:
            ...
            if await self.response_check(artists_responses, tasks_artists):
                ...
                for i, response in enumerate(artists_responses):
                    ...
                    if json_result["artists"]["items"]:
                        self.artists_ids.append(
                            json_result["artists"]["items"][0]["id"]
                        )
                    else:
            ...
            if await self.response_check(songs_info_responses2, tasks_songs_info2):
                ...
                sys.exit(
                    "Wychodzenie z programu przez brak pożądanej odpowiedzi od API"
                )
...
async def main():
    artists = validate(
        input("Podaj maksymalnie sześciu artystów, których chcesz wyszukać. Oddzielaj ich przecinkami: ")
    )
    start = time.time()
